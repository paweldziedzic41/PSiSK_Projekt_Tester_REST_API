...
l_errors = []
l_json_format = []
url_list = ["http://127.0.0.1:3000/drinks"]*1
...
body_get = 'GET', url_list[0] ,' GET data, [no cookies]'
body = str(body_get)

### Ustalamy cialo zapytania POST wysylanego na serwer
body_post = {"name": "pepsi","description" : "very goood"}
...
def get_method(url):
        session = get_session()
        try:
            with session.get(url,timeout=(3,0.5), headers=headers, data=body_get) as response:
                dane(response)
...
def post_method(url):
        session = get_session()
        try:
            with session.post(url,timeout=(3,0.5),headers=headers, json=body_post) as response:
                dane(response)
        except requests.exceptions.ReadTimeout as e:        
            l_errors.append(e)
        except requests.exceptions.ConnectionError as cr:
...
def download_all(urls):
    with ThreadPoolExecutor(max_workers=350) as executor:
        executor.map(post_method,url_list)
